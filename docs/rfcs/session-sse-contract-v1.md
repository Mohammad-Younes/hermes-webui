...
- This RFC does **not** modify `GET /api/sessions/events` (the existing global
  session-list invalidation stream routed at `api/routes.py:12355-12356` and
  implemented by `_handle_session_events_stream()` at `api/routes.py:16190`).
...
proposes. It is routed at `api/routes.py:12355-12356` and implemented by
`_handle_session_events_stream()` at `api/routes.py:16190`. It emits bare
...
`_SSE_HEARTBEAT_INTERVAL_SECONDS = 5` (`api/routes.py:1028-1036`) is the current
...
- `_parse_run_journal_event_id()` (`api/routes.py:15686-15698`) and
  `_parse_run_journal_after_seq()` (`api/routes.py:15701-15713`) parse the replay
  ...
  below, §Reconnect).
- `_runner_event_id()` at `api/routes.py:15778-15785` constructs the event `id`
  field as `stream_id:seq`.
- SSE frames carry their `id:` via the `_sse_with_id()` helper, emitted on the
  live `/api/chat/stream` path at `api/routes.py:15931`, on the runner-observe
  path at `api/routes.py:15824`, and during journal replay at
  `api/routes.py:15734` / `15747`.
- `_replay_run_journal()` reads events by `(session_id, stream_id)` at
  `api/routes.py:15716-15748`.
...
`api/routes.py:15778-15785`. Clients must treat it as an opaque string and must
...
`_replay_run_journal()` path (`api/routes.py:15716-15748`) and fall back to the
...
Phase 1 reuses `_SSE_HEARTBEAT_INTERVAL_SECONDS` (`api/routes.py:1028-1036`) for
